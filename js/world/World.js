...
import {Chunk} from "./Chunk.js?v=35";
import {Generator} from "./Generator.js?v=35";
import {BLOCK,INFO} from "./Block.js?v=35";
...
export class World{
  ...
  async generateAround(px,pz){const s=this.cfg.WORLD.CHUNK_SIZE,r=this.cfg.WORLD.RENDER_DISTANCE,cx=Math.floor(px/s),cz=Math.floor(pz/s),center=this.key(cx,cz);if(this.generationBusy||this.lastCenter===center)return false;this.lastCenter=center;this.generationBusy=true;const jobs=[];for(let x=-r;x<=r;x++)for(let z=-r;z<=r;z++)if(x*x+z*z<=r*r&&!this.chunks.has(this.key(cx+x,cz+z)))jobs.push([cx+x,cz+z]);jobs.sort((a,b)=>(a[0]-cx)**2+(a[1]-cz)**2-(b[0]-cx)**2-(b[1]-cz)**2);this.generationProgress={done:0,total:jobs.length,created:0};try{const concurrency=Math.max(1,Math.min(this.workers.length||1,this.cfg.QUALITY?.tier==='low'?1:2));for(let i=0;i<jobs.length;i+=concurrency){const batch=jobs.slice(i,i+concurrency);await Promise.all(batch.map(([x,z])=>this.generateChunk(x,z,true).then(()=>{this.generationProgress.done++;this.generationProgress.created++}).catch(err=>{this.generationProgress.done++;console.error('Chunk generation failed',x,z,err)})));await new Promise(requestAnimationFrame)}this.unloadFar(cx,cz,r+1);return true}finally{this.generationBusy=false;this.generationProgress=null}}
